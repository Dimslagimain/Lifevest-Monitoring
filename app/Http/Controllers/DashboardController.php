...
class DashboardController extends Controller
{
    public function __invoke()
    {
        ...
        $data = Cache::rememberForever($cacheKey, function () use ($lastSeatUpdate) {
            ...
            $cutoff = Carbon::createFromDate(2027, 4, 30)->endOfDay(); // Include all data up to end of April 2027

            // Pre-populate Weekly buckets for 52 weeks (~12 months)
            $weekStart = Carbon::createFromDate(2026, 5, 4)->startOfWeek(); // Starting week of the period
            for ($i = 0; $i < 52; $i++) {
            ...
            $monthStart = Carbon::createFromDate(2026, 5, 1)->startOfMonth();
            for ($i = 0; $i < 12; $i++) {
            ...
            foreach (['2026', '2027'] as $year) {
                $replacementPlans['yearly'][$year] = [
                    ...
                    'label' => $year,
                    'sort' => $year,
                    'start_date' => Carbon::createFromDate((int) $year, 1, 1)->startOfYear(),
                    'total' => 0,
                    'pn_breakdown' => [],
                    'aircraft_breakdown' => [],
                ];
            ...
            foreach (['weekly', 'monthly', 'yearly'] as $intervalName) {
                ...
                $replacementPlans[$intervalName] = array_filter(
                    ...
                    function ($bucket, $key) use ($currentKey) {
                        ...
                        if (strcmp($bucket['sort'], $currentKey) >= 0) {
                            return true;
                        }
            ...
            return [
                ...
                'lastUpdate' => $lastSeatUpdate ? Carbon::parse($lastSeatUpdate) : null,
                'pnSummary' => $pnSummary,
