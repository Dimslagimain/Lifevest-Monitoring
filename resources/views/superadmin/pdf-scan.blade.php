...
<div style="max-width: 800px; margin: 0 auto; padding: 0 1rem;">
    <div style="text-align: center; margin-bottom: 0.5rem;">
        ...
        @if (session('error'))
            <div
                ...
                <div
                    style="background: #ef4444; color: white; border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                        ...
                        <div class="upload-area" id="upload-area"
                            ...
                            <div id="file-info"
                                ...
                                <div style="display: flex; align-items: center; gap: 1.25rem;">
                                    <div
                                        style="background: rgba(16, 185, 129, 0.1); color: var(--success); width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center;">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <div style="text-align: left;">
                                        <h4 id="file-name"
                                            style="margin: 0 0 0.25rem 0; font-weight: 700; color: var(--text-primary); font-size: 1.05rem;">
                                            filename.jpg</h4>
                                        <p id="file-size"
                                            style="margin: 0; color: var(--text-muted); font-size: 0.85rem; font-weight: 500;">
                                            1.2 MB</p>
                                    </div>
                                </div>
                                <button type="button" id="remove-file" class="btn-icon"
                                    style="color: var(--danger); border-color: transparent;" title="Ganti File">
                            ...
                            <div id="file-error"
                                style="display: none; margin-top: 1rem; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.25); padding: 0.75rem 1rem; border-radius: 10px; text-align: left;">
                                <div style="display: flex; align-items: center; gap: 0.6rem;">
                                    ...
                                    <span id="file-error-msg"
                                        style="color: #ef4444; font-weight: 600; font-size: 0.85rem;"></span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Loading Overlay -->
    <div id="scan-loading-overlay"
        style="display: none; position: fixed; inset: 0; z-index: 9999; background: rgba(10, 10, 20, 0.92); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); justify-content: center; align-items: center; flex-direction: column; gap: 2rem;">
        <!-- Scanner Animation -->
        <div style="position: relative; width: 120px; height: 120px;">
            <!-- Outer ring -->
            <div id="scan-ring"
                style="position: absolute; inset: 0; border-radius: 50%; border: 3px solid transparent; border-top-color: var(--primary); animation: spin 1.2s linear infinite;">
            </div>
            <!-- Inner ring -->
            <div
                style="position: absolute; inset: 12px; border-radius: 50%; border: 3px solid transparent; border-bottom-color: rgba(var(--primary-rgb), 0.5); animation: spin 1.8s linear infinite reverse;">
            </div>
            <!-- Center icon -->
            <div style="position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;">
                <svg id="scan-center-icon" width="40" height="40" viewBox="0 0 24 24" fill="none"
        ...
        <div style="text-align: center;">
            <h2 style="margin: 0 0 0.5rem; font-size: 1.6rem; font-weight: 800; color: white; letter-spacing: -0.02em;">
                Mengekstrak Data...</h2>
            <p id="scan-subtitle" style="margin: 0; color: rgba(255,255,255,0.5); font-size: 0.95rem;">Mohon tunggu, AI
                multi-stage sedang membaca dokumen Anda</p>
        </div>
        ...
        <div style="width: 380px; max-width: 90vw; text-align: center;">
            ...
            <div
                style="width: 100%; height: 10px; background: rgba(255, 255, 255, 0.08); border-radius: 99px; overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.1); position: relative; box-shadow: inset 0 1px 2px rgba(0,0,0,0.4);">
                <!-- Inner Progress with Blue/Cyan Gradient and Glow -->
                <div id="scan-progress-bar"
                    style="width: 0%; height: 100%; background: linear-gradient(90deg, var(--primary), var(--primary-light)); border-radius: 99px; transition: width 0.15s linear; box-shadow: 0 0 12px rgba(var(--primary-rgb), 0.8), 0 0 4px rgba(var(--primary-rgb), 0.6);">
                </div>
            </div>
            <!-- Percentage & Current Status Text -->
            <div
                style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.75rem; padding: 0 0.25rem; gap: 1rem;">
                <span id="scan-status-text"
                    style="font-size: 0.85rem; font-weight: 600; color: rgba(255, 255, 255, 0.85); text-align: left; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">Mengunggah
                    file ke server...</span>
                <span id="scan-percentage-text"
                    style="font-size: 1rem; font-weight: 800; color: var(--primary); font-variant-numeric: tabular-nums; flex-shrink: 0;">0%</span>
            </div>
        </div>

        <!-- Elapsed Time -->
        <div style="text-align: center; margin-top: 0.5rem;">
            <span id="scan-elapsed"
                style="font-size: 0.8rem; color: rgba(255,255,255,0.3); font-variant-numeric: tabular-nums;">00:00</span>
            <p style="margin: 0.5rem 0 0; font-size: 0.75rem; color: rgba(255,255,255,0.2);">Multi-Stage OCR: proses
                biasanya 60-180 detik</p>
        </div>
    </div>
                ...
                ['dragenter', 'dragover'].forEach(eventName => {
                    uploadArea.addEventListener(eventName, () => {
                        ...
                        uploadArea.style.background =
                            'rgba(var(--primary-rgb), 0.05)'; // subtle teal tint
                    }, false);
                });
